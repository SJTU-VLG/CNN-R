...
def main():
    ...
    with tf.Session(config = config) as sess:
        ...
        features = np.zeros((n_classes, 256), dtype=np.float32)
        count = np.zeros((n_classes), dtype=np.int32)
        step = 1
        lr = init_lr
        while step < training_iters: 
            batch_xs, batch_ys = dataset.train.next_batch(batch_size)
            fea = sess.run(feature, feed_dict={X: batch_xs, Y: batch_ys, keep_var: keep_rate_train, learning_rate: lr, is_Training: False})
        ...
        for i in range(10000):
            ...
            fea = sess.run(feature, feed_dict={X: batch_ts, Y: batch_ty, keep_var: keep_rate_train, learning_rate: lr, is_Training: False})
            dis_min = -1
            ind = -1
            for i in range(n_classes):

...
def main():
    ...
    #prediction result
    # pred, pred2 = Model.residual_net(X, num, n_classes, is_Training)
    pred, pred2 = Model.simpleNet(X, keep_var, n_classes, is_Training)
    #loss and optimizer
    ...
    with tf.Session(config = config) as sess:
        print('Init variable!')
        if load_weight:
            saver.restore(sess, "./Model/LSTM/model_lstm_1.ckpt")
        else:
            sess.run(init)
        ...
        while step < training_iters: 
            ...
            sess.run(optimizer2, feed_dict={X: batch_xs, Y: batch_ys, keep_var: keep_rate_test, learning_rate: lr, is_Training: True})
            ...
            if step%test_step == 0:
                ...
                for _ in range(dataset.test.size//20):
                    ...
                    acc = sess.run(accuracy_2, feed_dict={X: batch_tx, Y: batch_ty, keep_var: keep_rate_test, is_Training: False})
                    test_acc += acc
                    test_count += 1
                test_acc /= test_count
            ...
            if step%display_step == 0:
                acc = sess.run(accuracy_2, feed_dict={X: batch_xs, Y: batch_ys, keep_var: keep_rate_test, is_Training: False})
                batch_loss = sess.run(loss_2, feed_dict={X: batch_xs, Y: batch_ys, keep_var: keep_rate_test, is_Training: False})
                print( "{} Iter {}: Training Loss = {:.4f}, Accuracy = {:.4f}".format(datetime.now(), step, batch_loss, acc))
            if step % 4000 == 0:
                saver.save(sess, './Model/LSTM/model_vgg_lstm_double_cell.ckpt')
            step += 1
        saver.save(sess, './Model/LSTM/model_vgg_lstm_double_cell.ckpt')
        print('finish!!!')
